...
(function (global) {
  ...
  var CASES = [
    {
      ...
      image: '',  // TODO: 配布後に img/voices/ 配下のパスを設定（例 'img/voices/case01.jpg'）。空なら非表示
  ...
  // ケース個別の画像（保護者アンケート等）。未指定/読込失敗なら何も表示しない。
  function caseImageHTML(c) {
    if (!c.image) return '';
    return '<div class="voice-photo"><img loading="lazy" src="' + esc(asset(c.image)) +
      '" alt="' + esc(uniLabel(c) + ' 合格 ' + c.initials + ' さん（保護者様の声）') +
