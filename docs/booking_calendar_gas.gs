 ...
 * - POST action=shindanLead: 診断後サンクスページの連絡先登録（診断用スプレッドシートの別タブに記録＋通知メール＋自動返信メール）
...
function saveShindanLeadToSheet(params) {
  ...
  if (!sheet) {
    sheet = ss.insertSheet(SHINDAN_LEAD_SHEET_NAME);
    sheet.appendRow([
...
function sendShindanLeadThankYou(params) {
  ...
  var body = [
    ...
    '',
    'なお、よりご状況に合ったロードマップを作成するため、必要に応じて現在の学習状況や目標について、別途ご連絡を差し上げる場合がございます。あらかじめご了承ください。',
    '',
    '━━━━━━━━━━━━━━━━━',
    '■ 学年: ' + (params.grade || ''),
    '■ 希望科目: ' + (params.subjects || ''),
